function MarketFlag(flagName) {
    if (Game.time % 20 != 0) return false;

    // 查找符合autobuy-{resourceType}-{amount}的flag
    const autobuyMatch = flagName.match(/^autobuy[-#/ ](\w+)[-#/ ](\d+)$/);
    if(autobuyMatch) {
        AutoBuy(autobuyMatch, flagName);
        return true;
    }

    // 查找符合autosell-{resourceType}-{amount}的flag
    const autosellMatch = flagName.match(/^autosell[-#/ ](\w+)[-#/ ](\d+)$/);
    if(autosellMatch) {
        AutoSell(autosellMatch, flagName);
        return true;
    }

    // 查找符合autodealbuy-{resourceType}-{amount}-{price}的flag
    const autodealbuyMatch = flagName.match(/^autodealbuy[-#/ ](\w+)[-#/ ](\d+)[-#/ ](\d+)$/)
    if(autodealbuyMatch) {
        AutoDealBuy(autodealbuyMatch, flagName);
        return true;
    }

    // 查找符合autodealsell-{resourceType}-{amount}-{price}的flag
    const autodealsellMatch = flagName.match(/^autodealsell[-#/ ](\w+)[-#/ ](\d+)[-#/ ](\d+)$/)
    if(autodealsellMatch) {
        AutoDealSell(autodealsellMatch, flagName);
        return true;
    }
    
}

function AutoBuy(autobuyMatch, flagName) {
    let resourceType = autobuyMatch[1];  // 资源类型
    const amount = parseInt(autobuyMatch[2]); // 资源数量, 自动购买阈值
...
function AutoSell(autosellMatch, flagName) {
    let resourceType = autosellMatch[1];  // 资源类型
    const amount = parseInt(autosellMatch[2]); // 资源数量, 自动出售阈值
    const flag = Game.flags[flagName];
    const room = flag.room;
